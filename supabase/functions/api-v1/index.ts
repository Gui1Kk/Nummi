...
function databaseError(error: { code?: string } | null, fallback: string): never {
  ...
  if (error?.code === "23503") {
    throw new ApiError(
      409,
      "INVALID_REFERENCE",
      "A referenced resource does not exist, is in use, or is not owned by this user"
    );
  }
...
async function loadSnapshot(
  ...
  const results = await Promise.all([
    ...
    client.from("transactions").select("*").eq("user_id", userId)
      ...
      .order("created_at", { ascending: false }).limit(5001),
...
async function summary(req: Request, client: SupabaseClient, url: URL, requestId: string) {
  assertQueryKeys(url, ["from", "to"]);
  const { from, to } = dateRange(url);
  const { data, error } = await client.rpc("get_financial_summary", {
    p_from: from,
    p_to: to
  });
  ...
  return json(req, 200, {
    data: {
      ...
      planned_expense: Number(row?.planned_expense ?? 0),
...
async function budgetStatus(req: Request, client: SupabaseClient, url: URL, requestId: string) {
  ...
  if (!month.endsWith("-01")) {
    throw new ApiError(400, "INVALID_MONTH", "month must be the first day of a month");
  }
  ...
  return json(req, 200, {
    data: (data ?? []).map((row: Record<string, unknown>) => ({
      ...
      effective_amount: Number(row.effective_amount),
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const route = `${req.method}:${path.slice(0, 2).join("/") || "health"}`;
    enforceRateLimit(userId, route, path[0] === "import" ? 10 : 120);
    ...
    if (path[0] === "snapshot" && req.method === "GET") {
      return loadSnapshot(req, client, userId, url, requestId);
    }
    if (path[0] === "profile") {
    ...
    if (path[0] === "summary" && req.method === "GET") {
      return summary(req, client, url, requestId);
    }
    if (path[0] === "budgets" && path[1] === "status" && req.method === "GET") {
      return budgetStatus(req, client, url, requestId);
    }
    if (path[0] === "notifications") {
      return notifications(req, client, userId, path[1], url, requestId);
    }
