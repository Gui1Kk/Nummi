...
import {
  automationInput, budgetInput, budgetPatch, categoryInput, categoryPatch,
  importInput, isoDate, notificationPatch, profilePatch, recurringInput,
  recurringPatch, settingsPatch, subscriptionInput, subscriptionPatch,
  transactionInput, transactionPatch, uuid
} from "./schemas-v2.ts";
...
const resources = {
  categories: { table: "categories", create: categoryInput, patch: categoryPatch, order: "name" },
  recurrences: { table: "recurring_rules", create: recurringInput, patch: recurringPatch, order: "next_date" },
  subscriptions: { table: "subscriptions", create: subscriptionInput, patch: subscriptionPatch, order: "next_charge" },
  budgets: { table: "budgets", create: budgetInput, patch: budgetPatch, order: "month" }
} as const;

function decodeJwtSubject(token: string) {
  const parts = token.split(".");
  if (parts.length !== 3) throw new ApiError(401, "UNAUTHORIZED", "Token is invalid");
  try {
    const normalized = parts[1].replaceAll("-", "+").replaceAll("_", "/");
    const padded = normalized.padEnd(Math.ceil(normalized.length / 4) * 4, "=");
    const payload = JSON.parse(atob(padded)) as Record<string, unknown>;
    const subject = uuid.safeParse(payload.sub);
    const expiration = Number(payload.exp);
    if (!subject.success || payload.role !== "authenticated"
      || !Number.isFinite(expiration) || expiration * 1000 <= Date.now()) {
...
function authenticate(req: Request) {
  ...
  // verify_jwt=true validates signature and expiration at the Supabase gateway.
  // The worker only extracts the already-validated subject to apply owner filters.
...
function enforceRateLimit(userId: string, route: string, limit = 120) {
  const now = Date.now();
  ...
  if (!current || current.resetAt <= now) {
    ...
    if (current.count >= limit) throw new ApiError(429, "RATE_LIMITED", "Too many requests");
    current.count += 1;
  }
...
function databaseError(error: { code?: string } | null, fallback: string): never {
  if (error?.code === "42501") throw new ApiError(403, "FORBIDDEN", "Operation is not allowed");
...
function dateRange(url: URL, maximumDays?: number) {
  ...
  if (maximumDays !== undefined) {
    ...
    if (days > maximumDays) throw new ApiError(400, "RANGE_TOO_LARGE", `Range cannot exceed ${maximumDays} days`);
  }
  return { from, to };
}
...
async function loadSnapshot(req: Request, client: SupabaseClient, userId: string, url: URL, requestId: string) {
  ...
  const { from, to } = dateRange(url, 366);
  const results = await Promise.all([
    ...
    client.from("budgets").select("*").eq("user_id", userId)
      .lte("month", `${to.slice(0, 7)}-01`).order("month", { ascending: false }).limit(2401),
    client.from("notifications").select("*").eq("user_id", userId)
      .is("read_at", null).order("created_at", { ascending: false }).limit(101)
  ]);
  ...
  const truncated = (categories.data?.length ?? 0) > 1000
    ...
    || (budgets.data?.length ?? 0) > 2400
    || (notifications.data?.length ?? 0) > 100;
  return json(req, 200, { data: {
    ...
    notifications: (notifications.data ?? []).slice(0, 100),
    from, to, truncated
  } }, requestId);
}
...
async function transactionCollection(req: Request, client: SupabaseClient, userId: string, url: URL, requestId: string) {
  ...
  if (req.method === "POST") {
    ...
    if (headerKey && !z.string().min(8).max(128).safeParse(headerKey).success) {
      throw new ApiError(400, "INVALID_IDEMPOTENCY_KEY", "Idempotency-Key must contain 8 to 128 characters");
    }
    const key = headerKey || parsed.idempotency_key || null;
    const { data, error } = await client.from("transactions")
      .insert({ ...parsed, idempotency_key: key, user_id: userId, source: "api" }).select().single();
...
async function summary(req: Request, client: SupabaseClient, url: URL, requestId: string) {
  ...
  return json(req, 200, { data: {
    from, to,
    income: String(row?.income ?? "0.00"),
    expense: String(row?.expense ?? "0.00"),
    balance: String(row?.balance ?? "0.00"),
    planned_income: String(row?.planned_income ?? "0.00"),
    planned_expense: String(row?.planned_expense ?? "0.00"),
    transaction_count: Number(row?.transaction_count ?? 0)
  } }, requestId);
}

async function budgetStatus(req: Request, client: SupabaseClient, url: URL, requestId: string) {
  assertQueryKeys(url, ["month"]);
  const month = isoDate.parse(url.searchParams.get("month"));
  ...
  return json(req, 200, { data: (data ?? []).map((row: Record<string, unknown>) => ({
    ...row,
    base_amount: String(row.base_amount ?? "0.00"),
    rollover_amount: String(row.rollover_amount ?? "0.00"),
    effective_amount: String(row.effective_amount ?? "0.00"),
    spent: String(row.spent ?? "0.00"),
    remaining: String(row.remaining ?? "0.00")
  })) }, requestId);
}
...
async function notifications(req: Request, client: SupabaseClient, userId: string, rawId: string | undefined, url: URL, requestId: string) {
  if (!rawId && req.method === "GET") {
    assertQueryKeys(url, ["limit", "offset", "unread"]);
    const page = pagination(url);
    ...
    const { data, error, count } = await query;
    if (error) databaseError(error, "Could not list notifications");
    return json(req, 200, { data, pagination: { ...page, total: count ?? 0 } }, requestId);
  }
  if (!rawId) throw new ApiError(405, "METHOD_NOT_ALLOWED", "Method not allowed");
  const id = uuid.parse(rawId);
  if (req.method === "PATCH") {
    ...
    const { data, error } = await client.from("notifications")
      .update({ read_at: patch.read ? new Date().toISOString() : null })
      .eq("id", id).eq("user_id", userId).select().maybeSingle();
    if (error) databaseError(error, "Could not update notification");
    if (!data) throw new ApiError(404, "NOT_FOUND", "Notification not found");
    return json(req, 200, { data }, requestId);
  }
  if (req.method === "DELETE") {
    const { data, error } = await client.from("notifications").delete()
      .eq("id", id).eq("user_id", userId).select("id").maybeSingle();
    if (error) databaseError(error, "Could not delete notification");
    if (!data) throw new ApiError(404, "NOT_FOUND", "Notification not found");
    return noContent(req, requestId);
  }
  throw new ApiError(405, "METHOD_NOT_ALLOWED", "Method not allowed");
...
async function exportTransactions(req: Request, client: SupabaseClient, userId: string, url: URL, requestId: string) {
  ...
  const csv = [columns.join(","), ...(data ?? []).map((row) => columns
    .map((column) => csvCell((row as Record<string, unknown>)[column])).join(","))].join("\r\n");
...
Deno.serve(async (req: Request) => {
  ...
  if (req.method === "OPTIONS") {
    if (!origin || !isAllowedOrigin(origin)) {
      return new Response(JSON.stringify({
        error: { code: "ORIGIN_NOT_ALLOWED", message: "Origin is not allowed" }, request_id: requestId
  ...
  try {
    if (!URL || !PUBLISHABLE_KEY) throw new ApiError(503, "MISCONFIGURED", "API is not configured");
    const url = new URL(req.url);
    const path = pathSegments(url);
    const { userId, client } = authenticate(req);
    enforceRateLimit(userId, `${req.method}:${path.slice(0, 2).join("/") || "health"}`, path[0] === "import" ? 10 : 120);
    ...
    if (path[0] === "transactions") return path[1]
    ...
    if (path[0] === "automations" && path[1] === "post-due" && req.method === "POST") {
      ...
      return json(req, 200, { data: data?.[0] ?? {
        transactions_created: 0, recurrences_advanced: 0, subscriptions_advanced: 0
      } }, requestId);
    }
    if (["categories", "recurrences", "subscriptions", "budgets"].includes(path[0] ?? "")) {
      return genericResource(req, client, userId, path[0] as Resource, path[1], url, requestId);
    }
    throw new ApiError(404, "NOT_FOUND", "Route not found");
  } catch (error) {
    if (error instanceof z.ZodError) {
      return fail(req, 400, "VALIDATION_ERROR", "Request validation failed", requestId,
