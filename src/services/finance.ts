...
function parseSnapshot(value:unknown):SnapshotResponse { const result=snapshotSchema.safeParse(value); if(!result.success)throw new Error("Resposta inválida da API em snapshot."); return result.data; }
...
export const authService={
  ...
  async updatePassword(currentPassword:string,password:string){const{data:userData,error:userError}=await supabase.auth.getUser();if(userError||!userData.user.email)authError(userError,"Não foi possível validar a sessão.");const{error:verifyError}=await supabase.auth.signInWithPassword({email:userData.user.email,password:currentPassword});if(verifyError)authError(verifyError,"A senha atual está incorreta.");const{error}=await supabase.auth.updateUser({password});if(error)authError(error,"Não foi possível alterar a senha.");},
...
export const financeService={
  async loadSnapshot():Promise<FinanceSnapshot>{const data=await apiRequest<unknown>("/snapshot");const parsed=parseSnapshot(data);if(parsed.truncated)throw new Error("Há dados demais para a visão completa. Use filtros ou a API paginada.");return{profile:parsed.profile,settings:parsed.settings,categories:parsed.categories,transactions:parsed.transactions,recurringRules:parsed.recurringRules,subscriptions:parsed.subscriptions,budgets:parsed.budgets};},
